...
let defaultExtensions = ['scss']; // расширения по умолчанию      ________[CUSTOM] Временно убрал расширение html, всегда можно добавить!________
  ...
  mkdirp(dirPath, function(err){                 // создаем
    ...
    // Нет ошибки, поехали!
      ...
      extensions.forEach(function(extention){
        ...
        let importContent = '';                               // Будущий импорт в диспетчере подключений
        ...
        if(fileExist(filePath) === false) {
          ...
          fs.appendFile('src/style.scss','\n' + styleFileImport, function(err) {          // [Custom]  Добавляем запись с импортом в главный файл стилей style.scss
            if(err) {
              throw err;
            }
            console.log('Файл импортирован в style.scss');
          });
        }
        else {
